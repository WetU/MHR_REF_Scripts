...
local function PreHook_applyEquipMySet(args)
    Constants.SendMessage(nil, InventorySupply.Restock(sdk.to_managed_object(args[2]) or sdk.get_managed_singleton("snow.data.EquipDataManager"), Constants.to_uint(args[3])));
end
...
local function campStart()
    Constants.SendMessage(nil, InventorySupply.Restock(sdk.get_managed_singleton("snow.data.EquipDataManager"), nil));
end
...
local function onVillageStart()
    ...
    if Constants.isOnVillageStarted ~= true then
        ...
        if AutoArgosy.autoArgosy() == true then
            Constants.SendMessage(ChatManager, "교역 아이템을 받았습니다");
        end
        Constants.SendMessage(ChatManager, InventorySupply.Restock(sdk.get_managed_singleton("snow.data.EquipDataManager"), nil));
    end
end
