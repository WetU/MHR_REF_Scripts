...
local function SendMessage(text)
    if config.config.EnableNotification then
...
Constants.SDK.hook(Constants.type_definitions.EquipDataManager_type_def:get_method("applyEquipMySet(System.Int32)"), function(args)
    ...
    if EquipDataManager then
        if setIdx ~= nil then
            ...
            local msg = InventorySupply.Restock(EquipDataManager, nil);
            SendMessage(msg);
        end
    else
        if setIdx ~= nil then
            local msg = InventorySupply.Restock(nil, setIdx);
            SendMessage(msg);
        else
            if config.config.Enabled then
...
Constants.SDK.hook(Constants.SDK.find_type_definition("snow.wwise.WwiseChangeSpaceWatcher"):get_method("onVillageStart"), nil, function()
    if config.config.EnableArgosy then
    ...
    if config.config.EnableCohootSupply then
...
Constants.RE.on_draw_ui(function()
    if Constants.IMGUI.tree_node("AutoSupply") then
        ...
        local changed = false;
        config_changed, config.config.Enabled = Constants.IMGUI.checkbox("Enabled", config.config.Enabled);
        changed, config.config.EnableNotification = Constants.IMGUI.checkbox("EnableNotification", config.config.EnableNotification);
        config_changed = config_changed or changed;
        changed, config.config.EnableCohoot = Constants.IMGUI.checkbox("EnableCohootSupply", config.config.EnableCohoot);
        config_changed = config_changed or changed;
        changed, config.config.EnableArgosy = Constants.IMGUI.checkbox("EnableArgosy", config.config.EnableArgosy);
        config_changed = config_changed or changed;

        local langChanged, new_langIdx = Constants.IMGUI.combo("Language", Constants.FindIndex(config.Languages, config.config.Language), config.Languages);
        config_changed = config_changed or langChanged;
        if langChanged then
            config.config.Language = config.Languages[new_langIdx];
        end

        changed, config.config.DefaultSet = Constants.IMGUI.slider_int("Default ItemSet", config.config.DefaultSet, 0, 39, InventorySupply.GetItemLoadoutName(config.config.DefaultSet));
        config_changed = config_changed or changed;

        if Constants.IMGUI.tree_node("WeaponType") then
            for i = 1, 14, 1 do
                local weaponType = i - 1;
                changed, config.config.WeaponTypeConfig[i] = Constants.IMGUI.slider_int(InventorySupply.GetWeaponName(weaponType), config.config.WeaponTypeConfig[i], -1, 39, InventorySupply.GetWeaponTypeItemLoadoutName(weaponType));
                config_changed = config_changed or changed;
            end
        ...
        if Constants.IMGUI.tree_node("Loadout") then
            for i = 1, 224, 1 do
                ...
                if name and InventorySupply.EquipmentLoadoutIsNotEmpty(loadoutIndex) then
                    ...
                    changed, config.config.EquipLoadoutConfig[i] = Constants.IMGUI.slider_int(name .. msg, config.config.EquipLoadoutConfig[i], -1, 39, InventorySupply.GetLoadoutItemLoadoutIndex(loadoutIndex));
                    config_changed = config_changed or changed;
                end
            end
            Constants.IMGUI.tree_pop();
